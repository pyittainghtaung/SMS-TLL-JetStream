                ...
                <div class="p-6 text-gray-900">
                    ...
                    <div class="flex justify-between flex-col sm:flex-row gap-2">
                        <div class="basis-1/3 rounded-lg border-solid border-2 border-green-500 p-2">
                            ...
                            @endif
                            <h1 class="text-center">{{ $isEdit ? 'Edit' : 'Create' }} Hostels</h1>
                            <form wire:submit.prevent="{{ $isEdit ? 'update' : 'store' }}">
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="text" wire:model="phone_no" class="border p-2 w-full">
                                    @error('phone_no')
                                        <span class="text-red-500 block">{{ $message }}</span>
                                    @enderror
                                </div>
                                ...
                                <tbody>
                                    @foreach ($hostels as $hostel)
                                        <tr wire:key="{{ $hostel->id }}">
                                            ...
                                            <td class="px-6 py-4">{{ $hostel->phone_no }}</td>
                                            <td class="px-6 py-4">
                                                <div class="flex gap-2">
                                                    <x-secondary-button wire:click="edit({{ $hostel->id }})"
                                                        class="btn btn-primary">Edit</x-secondary-button>
                                                    <x-secondary-button wire:click="delete({{ $hostel->id }})"
